...
def checkRunning(results, base_ankle, frame):
    global velocidad_actual, window
    h, w, _ = frame.shape
    # --- ahora usamos tobillos ---
    l_ankle = results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE].y * h
    r_ankle = results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE].y * h
    avg_ankle = (l_ankle + r_ankle) / 2

    now = time.time()

    # --- Caso 1: tobillos no se levantaron (quieto) ---
    if abs(avg_ankle - base_ankle) < 10:
        if len(window) == 0 or (now - last_steps[-1] if last_steps else 99) > 1.:
            velocidad_actual = 0
            return velocidad_actual

    # --- Caso 2: usar modelo LSTM ---
    ...
    if results.pose_landmarks:
        ...
        if hands == "Hands Joined" and (now - last_toggle_time > cooldown):
            if not empezo:
                ...
                l_ankle = results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE].y * h
                r_ankle = results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE].y * h
                base_ankle = (l_ankle + r_ankle) / 2
                print("[INIT] Línea hombros y tobillos guardados.")
            else:
                empezo = False
                linea_hombros, base_ankle = None, None
                velocidad_actual = 0
        ...
        if empezo:
            try:
                # validar tobillos visibles
                l_ankle_vis = results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE]
                r_ankle_vis = results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE]
                if l_ankle_vis.visibility < 0.5 or r_ankle_vis.visibility < 0.5:
                    raise ValueError("tobillos no visibles")
                ...
                if poshorizontal == "jumping":
                    ...
                    velocidad = checkRunning(results, base_ankle, frame)
                else:
                    velocidad = 0
